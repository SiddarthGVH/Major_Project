  ...
  if (loading) {
    return (
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-pulse">
        <div className="bg-white border border-slate-100 rounded-xl p-5 h-64 lg:col-span-2" />
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div className="bg-white border border-slate-150/60 rounded-xl p-5 shadow-sm/5 lg:col-span-2 flex flex-col justify-between hover:-translate-y-0.5 hover:shadow-md hover:border-slate-250 transition-all duration-300">
        <div>
          <div className="flex justify-between items-center mb-4">
            <h3 className="font-bold text-brand-text text-sm">Team performance</h3>
                  ...
                  return (
                    <tr key={index} className="hover:bg-slate-50/20 transition-colors">
                      ...
                      <td className="py-2.5 flex items-center space-x-2.5">
                        <div className="h-6.5 w-6.5 rounded-full overflow-hidden shrink-0 border border-slate-200">
                          <img src={member.avatar} alt={member.name} className="h-full w-full object-cover" />
                        </div>
                        <span className="font-bold text-brand-text truncate max-w-[120px]">{member.name}</span>
                      </td>
                      {/* Deals Won in Tabular Numbers */}
                      <td className="py-2.5 text-center text-slate-500 tabular-nums">{member.deals}</td>
                      {/* Revenue with Unified Blue Accent Bar */}
      ...
      <div className="bg-white border border-slate-150/60 rounded-xl p-5 shadow-sm/5 flex flex-col justify-between hover:-translate-y-0.5 hover:shadow-md hover:border-slate-250 transition-all duration-300">
        <div>
          <div className="flex justify-between items-center mb-4">
            <h3 className="font-bold text-brand-text text-sm">Activity overview</h3>
            <div className="relative">
              <select className="appearance-none bg-slate-55/60 border border-slate-200/80 text-slate-600 rounded-lg px-2.5 py-1 pr-7 text-[10px] font-bold focus:outline-none cursor-pointer">
              ...
              return (
                <div key={index} className="flex items-center justify-between py-1 hover:bg-slate-50/50 rounded-lg px-1.5 transition-colors">
                  <div className="flex items-center space-x-2.5">
                    ...
                    <span className="text-[11px] font-semibold text-slate-500">{act.name}</span>
                  </div>
